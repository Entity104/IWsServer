...
#include "tls/TlsRecords.h"
#include "libs/curve25519.h"
#include "libs/OpensslHeader.h"
#include "tls/EncFunctions.h"


//-----TlsRecordManager------
void TlsSession::RecordManager(byte data[5]){
...
//-----CipherSuitManager-----//
CipherSuites TlsSession::CipherSuiteChooser(std::vector<byte> ciphers){
    //Implement the choosing function:
    return TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA;
}
void TlsSession::CipherSuiteSetter(CipherSuites cipher, SignAlgorithem& sigAlgo){
    switch(cipher){
...
//-----TlsHandshakeManger-----//
void TlsSession::HandshakeManager(std::vector<byte>HandshakeData){
...
void TlsSession::HandshakeHandler_ClientHello(uint32_t lenght){
...
void TlsSession::HandshakeHandler_ClientKeyExchange(uint32_t lenght){
...
void TlsSession::HandshakeHandler_ClientHandshakeFinished(uint32_t lenght){
...
std::vector<byte> TlsSession::HandshakeHandler_ServerHello(CipherSuites chosenCipher){
...
std::vector<byte> TlsSession::HandshakeHandler_ServerCertificate(){
    //Generate the ServerCertificate
    ServerCertificate ServerCertificate_;

    return ServerCertificate_.GetData();
}
int TlsSession::HandhsakeHandler_ServerKeyGeneration(byte *public_key_, byte *private_key_){
...
std::vector<byte> TlsSession::HandshakeHandler_ServerKeyExchange(SignAlgorithem signAlgo){
...
std::vector<byte> TlsSession::HandshakeHandler_ServerHelloDone(){
    ServerHelloDone ServerHelloDone_;
    return ServerHelloDone_.GetData();
}
int TlsSession::HandshakeHandler_ServerEncryptionKeysCalculation(){
...
std::vector<byte> TlsSession::HandshakeHandler_ServerHandshakeFinished(){
    byte IV[16];
    RAND_bytes(IV, 16);
...
//-----TlsChangeCipherSpec-----
void TlsSession::ClientChangeCipherSpec(std::vector<byte>RecordData){
    std::cout << "[CLIENT " << sessionNumber << "] ClientChangeCipherSpec Received" << std::endl;
    clientChangeCipher = true;
    this->ASYNC_ReadRecordHeader();
}
std::vector<byte> TlsSession::ServerChangeCipherSpec(){
    ChangeCipherSpecClass ChangeCipherSpecClass_;
    return ChangeCipherSpecClass_.GetData();
}

//-----TlsApplicationData-----
void TlsSession::ApplicatonDataClient(std::vector<byte>ApplicationData){
...
void TlsSession::ApplicatonDataServer(){
...
//-----TlsSession-----
void TlsSession::ASYNC_ReadRecordHeader() {
...
void TlsSession::ASYNC_ReadBytes(std::size_t bytesToRead) {
    auto self(shared_from_this());
...
std::vector<byte> TlsSession::SYNC_ReadBytes(std::size_t bytesToRead) {
    auto self(shared_from_this());
...
void TlsSession::ASYNC_Write(std::vector<byte> responce){
    auto self(shared_from_this());
...
void TlsSession::CloseConnection(){
...
void TlsSession::StartReading(u_int64_t connectionNumber) {
